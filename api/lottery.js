...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 抽選実行
    const results = [];
    const directLogs = []; // バッティングあり抽選のログ（希望順位正確）
    const done = new Set();
      ...
      for (const [key, entries] of Object.entries(groups)) {
        ...
        if (uniq.length > 1) {
          directLogs.push({
            ...
            time_slot: timeSlot,
            pref_index: prefIdx,
            candidates: uniq.map(e => ({ id: e.cand_id, name: e.name, party: e.party })),
            winner_id: winner.cand_id
          });
        }
    ...
    // lottery_logsに記録（directLogsを使用：希望順位が正確）
    if (directLogs.length > 0) {
      await sbPost('lottery_logs', directLogs);
    }
